...
def test9():
        """
        implementation of CNN with Xavier initialization and only one epoch
        layers: conv1->ReLU->maxpool->conv2->ReLU->maxpool->Flatten->linear->Softmax->CrossEntropy
        dimension: 28*28*1->24*24*16->12*12*16->8*8*32->4*4*32->10
        optimizer: SGD
        ...
...
def test11():
        """
        implementation of CNN with data augmentation, which doubles the dataset
        layers: conv1->maxpool->conv2->maxpool->linear->Softmax->CrossEntropy
        dimension: 28*28*1->26*26*32->13*13*32->11*11*64->5*5*64->10
        optimizer: SGD
        ...
...
def test12():
        """
        implementation of CNN with bigger kernel and data augmentation, which doubles the dataset
        layers: conv1->maxpool->conv2->maxpool->linear->Softmax->CrossEntropy
        dimension: 28*28*1->24*24*16->12*12*16->8*8*32->4*4*32->10
        optimizer: SGD
        ...
...
if args.test in test_mapping:
        print(f"--- test{args.test} is running... ---")
        test_mapping[args.test]()
        print(f"--- test{args.test} finished! ... ---")
else:
